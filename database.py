...
class SQLiteDB:
    ...
    def setup_database(self):
        self.cursor.execute('CREATE TABLE IF NOT EXISTS api_keys (api_key TEXT PRIMARY KEY, name TEXT UNIQUE)')
        ...
        if admin is None:
            ...
            if api_key is None:
                exit("Error when generating an Admin API key. The DB failed to create an Admin API key despite one not existing. This should NEVER happen!")
    ...
    def add_api_key(self, name: str) -> str:
        ...
        try:
            ...
            self.cursor.execute('INSERT INTO api_keys (api_key, name) VALUES (?, ?)', (new_api_key, name))
